function perm_table = createPermTable(params_cell)
...
% ai.default params to make a run parameter table
    ...
    perm_table(:,1) = reshape(repmat(params_cell{1},N(2),1),N(1),N(1)/(N(2)*n(1)));
    for j = 2:nvar
        perm_table(:,j) = reshape(repmat(params_cell{j},N(j+1),N(1)/(n(j)*N(j+1))),N(1),1);
    end
