...
C1 = X(:,taxaIndex(1,taxa_id),:,:,:)+X(:,taxaIndex(2,taxa_id),:,:,:);%Adding enhanced to unenhanced tabular Acropora 
C2 = X(:,taxaIndex(3,taxa_id),:,:,:) + X(:,taxaIndex(4,taxa_id),:,:,:);%Adding %enhanced to unenhanced corymbose Acropora 
C3 = X(:,taxaIndex(5,taxa_id),:,:,:);%Encrusting and small massives 
C4 = X(:,taxaIndex(6,taxa_id),:,:,:);%Large massives
