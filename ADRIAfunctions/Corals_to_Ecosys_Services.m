function [CultES, ProvES, dCultES, dProvES,Nint] = Corals_to_Ecosys_Services(RCP,alg_ind)
%% load data file
cd ../Outputs

%loads results from ADRIA - 
F0 = load(strcat('Results_RCP',num2str(RCP),'_Alg',num2str(alg_ind)));

TC = F0.TC; C = F0.C; E = F0.E; S = F0.S;
...
filename =  strcat('ESresults_RCP', num2str(RCP),'_Alg', num2str(alg_ind), '.mat');

save(filename, 'CultES','ProvES','dCultES','dProvES','Nint');
end