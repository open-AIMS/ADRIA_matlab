...
%[TPdata,SiteRanks,strongpred] =  ADRIA_TP_Moore(showfigs,con_cutoff); %con_cutoff filters out low connectivities
[TPdata,SiteRanks,strongpred,nsites] =  ADRIA_TP_Moore(params.con_cutoff); %con_cutoff filters out low connectivities
%TPforecast = randi(nTPstates,[tf,sims]); %create uniformly distributed
%pseudo-random integers (environmental states) - NOT USED NOW

%% Simulate future wave exposure patterns from Puotinen and Callaghan SWH data
swhtbl = readtable('Inputs/swhMoore_interp.xlsx', 'PreserveVariableNames',true); %import Marji's significant wave heights 
...
F = load('Inputs/MooreDHWs'); %load data that are previously generated
z = F.z; %bathymetry
...
for sim = 1:Interv.sims
    dhwdisttime(:,:,sim) = ADRIA_DHWprojectfun(params.tf,nsites,mdhwdist0,...
        sdhwdist0,params.dhwmax25,RCP,params.wb1,params.wb2);
end
