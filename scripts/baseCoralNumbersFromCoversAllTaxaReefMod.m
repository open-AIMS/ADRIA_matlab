function base_cover = baseCoralNumbersFromCoversAllTaxaReefMod(target_covers)
...
base_cover(2,:) = ((target_covers(2) / 100) ./ conversion_vector(:,2) .* cover_factor)';
base_cover(3,:) = zeros(1,6);
base_cover(4,:) = ((target_covers(3) / 100) ./ conversion_vector(:,4) .* cover_factor)';
base_cover(5,:) = ((target_covers(5) / 100) ./ conversion_vector(:,5) .* cover_factor)';

% Preliminarily set start cover for Large Massive to 5 percent
base_cover(6,:) = ((target_covers(6) / 100.0) ./ conversion_vector(:,6) .* cover_factor)';
